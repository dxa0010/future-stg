...
export class Ui {
  ...
  private lastWeapon: WeaponId = 'gatling';

  constructor(private readonly cb: UiCallbacks) {
    // 解放はタップの瞬間に効く。ドラッグ扱いにならないよう伝播を止める。
  ...
  // ------------------------------------------------------------- 共通
  private open(html: string, bottom = false): void {
  ...
  private on(id: string, fn: () => void): void {
    const el = this.screen.querySelector<HTMLElement>(`#${id}`);
  ...
  // ------------------------------------------------------------- タイトル
  showTitle(defaultSeed: string): void {
    this.open(`
      ...
      <div class="help">
        <b>ドラッグ</b>：移動＋自動射撃<br />
        <b>フリック</b>：攻めの回避。無敵中に敵弾と重なると<b>ジャスト</b>成立 → 瞬炎<br />
        <b>指を離す</b>：完全静止して溜め。<b>溜めは繰り越せる</b>が、被弾すると全消滅<br />
        <b>解放ボタン</b>：臨界共鳴（極太ビーム＋弾消し衝撃波＋ジェム吸着）<br />
        <span class="sub">PC：WASD/矢印で移動、Shift+方向でフリック、Space で解放</span>
      </div>
    ...
    const start = (w: WeaponId): void => {
      this.lastWeapon = w;
      this.cb.onStart(w, seedInput?.value ?? defaultSeed);
    };
    ...
    this.on('snd', () => {
      ...
      const el = this.screen.querySelector('#snd');
      if (el) el.textContent = muted ? '音 OFF' : '音 ON';
    });
    this.on('crtbtn', () => {
      const on = this.cb.onToggleCrt();
      const el = this.screen.querySelector('#crtbtn');
      if (el) el.textContent = on ? 'CRT ON' : 'CRT OFF';
    });
  }

  // ------------------------------------------------------------- レベルアップ
  showLevelUp(choices: Choice[], level: number): void {
    const cards = choices
      .map(
        (c, i) => `
  ...
  showPause(): void {
    ...
    this.on('snd', () => {
      const muted = this.cb.onToggleSound();
      this.toast(muted ? '音 OFF' : '音 ON', 700);
    });
    this.on('crtbtn', () => {
      const on = this.cb.onToggleCrt();
      this.toast(on ? 'CRT ON' : 'CRT OFF', 700);
    });
  }
